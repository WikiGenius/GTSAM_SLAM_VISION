...
classdef NonlinearEqualityRot3 < gtsam.NoiseModelFactor
  ...
  methods
    function obj = NonlinearEqualityRot3(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(1929, varargin{2});
        end
        base_ptr = gtsam_wrapper(1928, my_ptr);
      elseif nargin == 2 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.Rot3')
        [ my_ptr, base_ptr ] = gtsam_wrapper(1930, varargin{1}, varargin{2});
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.Rot3') && isa(varargin{3},'double')
        [ my_ptr, base_ptr ] = gtsam_wrapper(1931, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gtsam_wrapper(1932, obj.ptr_gtsamNonlinearEqualityRot3);
    end
  ...
  methods(Static = true)
    function varargout = string_deserialize(varargin)
      ...
      if length(varargin) == 1
        varargout{1} = gtsam_wrapper(1944, varargin{:});
      else
