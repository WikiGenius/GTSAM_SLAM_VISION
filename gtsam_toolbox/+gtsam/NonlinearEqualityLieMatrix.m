...
%string_deserialize(string serialized) : returns NonlinearEqualityLieMatrix
%
classdef NonlinearEqualityLieMatrix < gtsam.NoiseModelFactor
  properties
  ...
  methods
    function obj = NonlinearEqualityLieMatrix(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(1844, varargin{2});
        end
        base_ptr = gtsam_wrapper(1843, my_ptr);
      elseif nargin == 2 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.LieMatrix')
        [ my_ptr, base_ptr ] = gtsam_wrapper(1845, varargin{1}, varargin{2});
    ...
    function delete(obj)
      gtsam_wrapper(1847, obj.ptr_gtsamNonlinearEqualityLieMatrix);
    end
    ...
    function varargout = equals(this, varargin)
      ...
      if length(varargin) == 2 && isa(varargin{1},'gtsam.NonlinearFactor') && isa(varargin{2},'double')
        gtsam_wrapper(1851, this, varargin{:});
      else
    ...
    function varargout = print(this, varargin)
      ...
      if length(varargin) == 1 && isa(varargin{1},'char')
        gtsam_wrapper(1855, this, varargin{:});
      else
    ...
    function varargout = string_serialize(this, varargin)
      ...
      if length(varargin) == 0
        ...
        error('Arguments do not match any overload of function gtsam.NonlinearEqualityLieMatrix.string_serialize');
      end
    end
  ...
  methods(Static = true)
    function varargout = string_deserialize(varargin)
      % STRING_DESERIALIZE usage: string_deserialize() : returns gtsam.NonlinearEqualityLieMatrix
      % Doxygen can be found at http://research.cc.gatech.edu/borg/sites/edu.borg/html/index.html
      if length(varargin) == 1
        varargout{1} = gtsam_wrapper(1859, varargin{:});
      else
        error('Arguments do not match any overload of function gtsam.NonlinearEqualityLieMatrix.string_deserialize');
      end
    end

    function obj = loadobj(sobj)
      % LOADOBJ Saves the object to a matlab-readable format
      obj = gtsam.NonlinearEqualityLieMatrix.string_deserialize(sobj);
    end
  end
end
